...
class MLPCls:
    ...
    def __init__(self, layers, epochs, learning_rate = .2 , show_training_plot = False):
        """
        A multilayer feed forward network that performs classification tasks.
        
        Parameters:
        -------------
        layers : array of ints. The number of values will determine the number of nodes at each layer. The output layer needs to match up with the number of classes. The inuput layer must match up with the dimension of the dataset 
    ...
    def fit(self, X, y, validation_X, validation_y):
        ...
        for _ in range(self.epochs):
            ...
            # let's first grab what the final output is supposed to be
                ...
                y_t = np.zeros_like(self.classes, dtype=int)
                y_index = self.classes.index(y.iloc[point_index])
                
                # flip the real value to 1
                y_t[y_index] = 1
    ...
    def backprop_(self, outputs, y_t):
        ...
        for layer_index, output in reversed(list(enumerate(outputs))):
            ...
            # the output layer is a special case since we have to push the resulting outputs through softmax
            ...
            if layer_index == len(outputs) - 1:
                ...
                # update for classification is the same as regrsession for backprop
    ...
    def propagate_(self, point):
        ...
        for layer_index, layer in enumerate(self.weights):
            # at the final layer we have probabilities for outputs.
            if layer_index == len(self.weights) - 1:
                output = layer @ inputs + self.biases[layer_index]
    ...
    def predict(self, X):
        ...
        output = self.propagate_(X)[-1]
        output_index = np.argmax(output)
        return self.classes[output_index]
